...
static void CurlLog(NSString *format, ...) {
    ...
    NSLog(@"[RemoteCompanion] [NativeCurl] %@", message);
    
    // Write to same log file
    NSString *logMsg = [NSString stringWithFormat:@"%@ [RemoteCompanion] [NativeCurl] %@\n", [NSDate date], message];
...
static void perform_native_curl(NSString *fullCmd) {
    ...
    CurlLog(@"[RemoteCompanion] Native Curl: %@ %@ (insecure=%d, auth=%@)", method, urlStr, insecure, basicAuth ? @"YES" : @"NO");
