...
typedef unsigned int MRMediaRemoteCompanion;
extern Boolean MRMediaRemoteSendCommandToApp(MRMediaRemoteCompanion command, NSDictionary *userInfo, id origin, NSString *bundleIdentifier, unsigned int options, dispatch_queue_t queue, void (^completion)(NSError *));
...
void SRLog(NSString *format, ...) {
    ...
    NSLog(@"[RemoteCompanion] %@", message);
    
    NSString *logMsg = [NSString stringWithFormat:@"%@ [RemoteCompanion] %@\n", [NSDate date], message];
    NSFileHandle *fileHandle = [NSFileHandle fileHandleForWritingAtPath:@"/tmp/remotecompanion.log"];
    if (fileHandle) {
        @try {
        ...
        [logMsg writeToFile:@"/tmp/remotecompanion.log" atomically:YES encoding:NSUTF8StringEncoding error:nil];
    }
}
...
static void inject_hid_event(uint32_t page, uint32_t usage, uint64_t durationNs, IOOptionBits flags) {
    ...
    dispatch_once(&onceToken, ^{
        hidQueue = dispatch_queue_create("com.remote.companion.hid", DISPATCH_QUEUE_SERIAL);
...
static NSString *handle_command(NSString *cmd) {
    ...
    if ([cleanCmd isEqualToString:@"pause"]) {
        ...
        NSString *title = @"RemoteCompanion";
        NSString *body = @"";
        BOOL urgent = NO;
        ...
        if ([appName containsString:@"."]) {
             NSLog(@"[RemoteCompanion] Opening Bundle ID via FBS: %@", appName);
             dispatch_async(dispatch_get_main_queue(), ^{
        ...
        NSLog(@"[RemoteCompanion] Opening app: %@ via %@", appName, urlString);
        dispatch_async(dispatch_get_main_queue(), ^{
            NSURL *url = [NSURL URLWithString:urlString];
            if (url) {
                [[UIApplication sharedApplication] openURL:url options:@{} completionHandler:nil];
        ...
        if (bbHandle) {
            ...
            if (BKSDisplayBrightnessSet) {
                ...
                NSLog(@"[RemoteCompanion] Brightness set to: %.0f%%", value * 100);
            }
        }
        ...
        NSLog(@"[RemoteCompanion] Haptic triggered");
        ...
        if ([device hasTorch] && [device isTorchAvailable]) {
            ...
            [device setTorchMode:AVCaptureTorchModeOn];
            [device unlockForConfiguration];
            NSLog(@"[RemoteCompanion] Flashlight on");
        }
        ...
        if ([device hasTorch]) {
            ...
            [device setTorchMode:AVCaptureTorchModeOff];
            [device unlockForConfiguration];
            NSLog(@"[RemoteCompanion] Flashlight off");
        }
        ...
        if ([device hasTorch] && [device isTorchAvailable]) {
            ...
            if ([device torchMode] == AVCaptureTorchModeOn) {
                [device setTorchMode:AVCaptureTorchModeOff];
                NSLog(@"[RemoteCompanion] Flashlight toggled OFF");
            } else {
                [device setTorchMode:AVCaptureTorchModeOn];
                NSLog(@"[RemoteCompanion] Flashlight toggled ON");
            }
            [device unlockForConfiguration];
        }
    } else if ([cleanCmd isEqualToString:@"flashlight on"]) {
...
static void start_server() {
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        if ((server_fd = socket(AF_INET, SOCK_STREAM, 0)) == 0) {
            SRLog(@"[RemoteCompanion] ERROR: Failed to create socket");
            return;
        }

        // Set both SO_REUSEADDR and SO_REUSEPORT for faster rebind after respring
        if (setsockopt(server_fd, SOL_SOCKET, SO_REUSEADDR, &opt, sizeof(opt))) {
            SRLog(@"[RemoteCompanion] WARNING: Failed to set SO_REUSEADDR");
        }
        #ifdef SO_REUSEPORT
        if (setsockopt(server_fd, SOL_SOCKET, SO_REUSEPORT, &opt, sizeof(opt))) {
            SRLog(@"[RemoteCompanion] WARNING: Failed to set SO_REUSEPORT");
        }
        #endif
        ...
        for (int i = 0; i < num_ports; i++) {
            ...
            if (bind(server_fd, (struct sockaddr *)&address, sizeof(address)) == 0) {
                bound_port = ports[i];
                SRLog(@"[RemoteCompanion] Successfully bound to port %d", bound_port);
                break;
            } else {
                SRLog(@"[RemoteCompanion] Failed to bind to port %d (errno: %d - %s), trying next...", 
                      ports[i], errno, strerror(errno));
            }
        }
        
        if (bound_port == 0) {
            SRLog(@"[RemoteCompanion] ERROR: Failed to bind to any port!");
            close(server_fd);
            return;
        }
        
        if (listen(server_fd, 3) < 0) {
            SRLog(@"[RemoteCompanion] ERROR: Failed to listen (errno: %d)", errno);
            close(server_fd);
            return;
        }

        SRLog(@"[RemoteCompanion] Server listening on port %d", bound_port);
